...
- **アクセシビリティ**:
  ...
  - **AI思考中スピナー**: AIが考え中のとき視覚的フィードバック（スピナー表示）
  - **トースト通知**: 移動・エラーを画面上部に視覚トーストで通知（聴覚障害対応）
  - **盤面差分読み上げ**: Shift+Bで前回からの変化のみ読み上げ（有段者向け）
  - **利き筋情報**: Iキーで現在マスを狙う相手の駒を読み上げ
  - **ショートカットヒント**: 画面内に主要キーボードショートカットを常時表示
  - **表示設定**: 駒の文字サイズ変更（小/標準/大/特大）・タイマー非表示オプション
  - **prefers-reduced-motion対応**: OS設定に応じてアニメーション無効化
