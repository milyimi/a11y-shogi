...
- **アクセシビリティ**:
  ...
  - キーボードのみで全操作可能（矢印キー + WASD代替ナビゲーション、駒台操作: Shift+T/G または 1/2キー）
  - **合法手ハイライト**: 駒選択時に移動可能マスを緑色ドットで視覚表示 + aria-liveで合法手一覧読み上げ
  - **AIハイライト**: AI最終手を★マーカーで表示（通常モード・ハイコントラスト両対応）
  - **AI思考中スピナー**: AIが考え中のとき視覚的フィードバック
  - **トースト通知**: 移動・エラーを画面上部に視覚トーストで通知（無効化可能）
  - **盤面差分読み上げ**: Shift+Bで前回からの変化のみ読み上げ
  - **利き筋情報**: Iキーで現在マスを狙う相手の駒を読み上げ
  - **駒選択状態バー**: 選択中の駒とEscape解除方法を常時表示
  - **表示設定**: 駒の文字サイズ変更・タイマー非表示・トースト無効化・フォント変更（モーダルダイアログで表示）
  - **モーダルダイアログ**: 棋譜・表示設定・ショートカットをモーダル化（フォーカストラップ・Escape閉じ・フォーカス復帰対応）
  - **ランキングダイアログ**: 勝利時のランキング登録ダイアログに閉じるボタン付き（他モーダルとUI統一）
  - **ランドマーク構造**: ページごとに一意の`<main>`要素（HTML仕様準拠）
  - **ゲーム情報ライブ更新**: 手番・手数・経過時間セクションに`aria-live="polite"`適用
  - **レイアウト**: 駒台を盤面の左右に配置（後手=左上・先手=右下、伝統的な自分の右側配置）
  - **駒台拡大**: 駒台サイズ180×280px以上、駒ボタンの自然な高さ表示（stretch防止）
  - **prefers-reduced-motion対応**: OS設定に応じてアニメーション完全停止（スピナー非表示化）
  - **ボタン分離設計**: 投了ボタンを分離配置し誤操作防止
  - **ボタンデザイン**: 将棋盤の木目調グラデーション・焦げ茶文字・温かみのある配色（投了ボタンは赤色で危険操作を視覚的に区別）
  - **ツールチップ**: 将棋用語にマウスホバー説明表示（木目調統一・forced-colorsモード対応）
  - **disabledボタン**: 点線枠+グレースケールで視覚的に無効表示 + aria-describedbyで「まだ使えません」を読み上げ
  - **AI長考再通知**: 5秒以上AI思考中の場合「もう少しお待ちください」と再通知
  - **ヘルプページ**: キーボードショートカットを`<kbd>`要素でセマンティックにマークアップ（木目調スタイル付き）
  - **ふりがな対応**: 駒名・ボタン名にルビを付与（投了→とうりょう）
  - **難易度補足**: 「初級（よわい）」のように平易な説明付き
  - **片手操作対応**: 1/2キーで駒台移動（Shift同時押し不要）
  - **キー入力デバウンス**: 手の震えによる誤操作を防止
  - **フォント変更**: UDデジタル教科書体・BIZ UDゴシック選択可能
...
- **Testing**: Pest (125 tests), Puppeteer (59 E2E + 30 contrast + 77 blind user + 59 full-feature)
...
| テストスイート | テスト数 | 内容 |
|---|---|---|
| PHP (Pest) | 125 | ルール検証、AI評価、成り、ボード境界、エラーハンドリング等 |
