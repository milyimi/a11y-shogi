...
- **3画面フロー**: 入力 → 確認 → 送信完了（誤送信防止）
- **レート制限**: セッション × IP で5分間隔制限（スパム防止）
- **日本語バリデーション**: 日本語・英数字・記号に対応した入力検証
- **日本語ラベル化**: メール内容を日本語で自然に表示
- **GitHub Copilot推奨**: AIツール使用時の発見に期待するメッセージ
- **アクセシブルなフォーム**: 全項目にaria-label、エラーはrole="alert"でアナウンス
- **フィードバック種別**: 不具合報告・機能リクエスト・一般的なご意見
...
|---|---|---|---|
| PHP (Pest)  | `npm run test:php` | 140 | ルール検証、AI評価、成り・持ち駒、ボード境界等 |
| **アクセシビリティテスト（8種）** | `npm run test:a11y` | 227 | 視覚障害・運動障害・環境対応。以下の8種類：|
...
| **AIペルソナテスト（10種）** | `npm run test:ai` | 364 | 障害者・年齢層・環境・專門家を網羅。詳細は以下：|
...
- 24種類のペルソナ（障害者17種類 + AI7種類）による品質検証 0 failures ✅
