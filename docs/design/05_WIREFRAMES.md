...
││金x0     ││  [9] [8] [7] [6] [5]   ││金x0     │││                      │ │
││角x0     ││                          ││角x0     │││ 棋譜:                │ │
││飛x0     ││  現在位置: 5-5-1       ││飛x0     │││ 1. 7g-7f 3c-3d     │ │
││        ││                          ││        │││ 2. 8h-7g 8c-8d     │ │
││(上部)  ││  合法手:                ││(下部)  │││ 3. 7g-7f 4c-4d     │ │
││(右寄り)││  7f, 8h, 6h, 2h        ││(左寄り)│││ ...                 │ │
││        ││                          ││        │││                      │ │
│└─────────┘│                          │└─────────┘│ 現在の手:            │ │
│          │                          │          │ 後手を考え中...      │ │
...
2. 盤面エリア（role="main"）← 最も重要
   - role="board" で盤面を識別
   - 先手の駒台（role="region" aria-label="先手の駒台"）
   - 実際の盤（9×9グリッド）
   - 後手の駒台（role="region" aria-label="後手の駒台"）
   
3. 棋譜エリア（role="region" aria-label="棋譜"）
   - 過去の指し手一覧
   - aria-live="polite" で更新を通知

4. ゲーム状態（role="status" aria-live="polite" aria-atomic="true"）
   - 現在のターン、AI思考状況など
```
...
.sente-komadai { grid-column: 1; grid-row: 1; } /* 左上 */
.board { grid-column: 2; grid-row: 1 / 3; }     /* 中央全体 */
.game-info { grid-column: 3; grid-row: 1 / 3; } /* 右全体 */
.gote-komadai { grid-column: 3; grid-row: 3; }  /* 右下 */
```
...
- テーブル: `<table>` セマンティックタグ使用
  - `<caption>`: "中級ランキング"
  - `<th scope="col">`: 各列ヘッダー
- スクリーンリーダー: テーブル全体を効率的に読み上げ
- キーボード操作: Tab で難易度切り替え、↑↓ でランキング移動
- 自分の順位がある場合はハイライト表示
...
4カラムレイアウト（「3. 対局画面 - 全体レイアウト」参照）
- 先手駒台（左1） | 盤面（中央2） | 後手駒台（右3） | ゲーム情報&棋譜（右4）

### タブレット（768px 〜 1199px）

3カラムレイアウト（先手駒台 | 盤面 | 後手駒台＆情報）
```
┌────────────────┬──────────────────────────┬─────────────────┐
│  a11y-shogi                                            ≡     │
├────────────────┴──────────────────────────┴─────────────────┤
...
**レスポンシブ設計ポイント**
- **デスクトップ（1200px以上）**: 4カラム（先手駒台 | 盤面 | 後手駒台 | ゲーム情報）で効率的に情報表示
- **タブレット（768px-1199px）**: 3カラム（先手駒台 | 盤面 | 後手駒台＆情報）に集約
- **スマートフォン（-767px）**: 1カラム（縦並び）で、アコーディオン形式で開閉
- 盤面は常に正方形で表示
- タッチ操作時のボタン最小サイズ: 44×44px
- 小画面でのアクセシビリティ優先（キーボード操作重視）
