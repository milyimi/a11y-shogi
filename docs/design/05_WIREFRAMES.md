...
├─────────────────────────────────────────────┤
│  🎮 はじめての方へ                          │ guide box
│  キーボードだけで操作できます。矢印キーで   │
│  盤面を移動し、Enterで駒を選択・移動します。│
│  B/Sキーで盤面・状態の読み上げができます。  │
│  [詳しい操作方法を見る]                      │ link → /help
├─────────────────────────────────────────────┤
...
## 3. 対局画面 - 全体レイアウト（デスクトップ：2カラム）

```
┌──────────────────────────────────────────────────────────────────────────┐
│  a11y-shogi - 対局画面                                               ≡    │ header
├──────────────────────────────────────────┬──────────────────────────────┤
│  盤面エリア (.board-area)                │ ゲーム情報                   │
│                                          │┌──────────────────────────┐ │
│ 後手駒台    盤 面 (9×9)    先手駒台      ││ 難易度: 中級              │ │
│┌────────┐                 ┌────────┐    ││ 先手（あなた）            │ │
││歩x2    │ ☖香☖桂☖銀☖金 │歩x3    │    ││ 第 5 手                  │ │
││香x0    │ ☖角    ☖飛    │香x0    │    ││ 時間: 02:30              │ │
││桂x0    │ ☖歩☖歩☖歩    │桂x0    │    ││                          │ │
││銀x0    │                 │銀x1    │    │└──────────────────────────┘ │
││金x0    │                 │金x0    │    │                            │
││角x0    │  [9][8][7]...   │角x0    │    │ [📋 棋譜]      → モーダル  │
││飛x0    │                 │飛x0    │    │ [⚙ 表示設定]    → モーダル  │
│└────────┘ ☗歩☗歩☗歩    └────────┘    │ [⌨ ショートカット] → モーダル│
│            ☗飛    ☗角                   │                            │
│            ☗金☗銀☗桂☗香               │ ┌──────────┬──────────┐   │
...
**2カラム + board-area flexレイアウトの利点**
- **伝統的配置**: 駒台は自分の右側に置くのが一般的（先手=右下、後手=左上）
- **盤面一体化**: 駒台と盤面が横並びで視認性向上
- **info-panel圧縮**: 棋譜・設定・ショートカットをモーダル化して高さ削減
- **ズーム対応**: 200%拡大でも横スクロール発生しにくい
- **視線移動**: 左（後手駒台）→ 中央（盤面）→ 右（先手駒台＆情報）と自然な流れ

**モーダルダイアログのアクセシビリティ**
- `role="dialog"` + `aria-modal="true"` で支援技術に通知
- `aria-labelledby` でモーダルタイトルと紐付け
- フォーカストラップ（Tab循環）で外部要素への意図しないフォーカス移動を防止
- `Escape` キーで閉じる（ゲーム操作のEscapeよりも優先）
- 閉じた後、開いたボタンへフォーカスを復帰
- オーバーレイクリックでも閉じ可能
- `body` のスクロールをロックして背景操作を防止
...
2. 盤面エリア（role="main"）← 最も重要
   - 後手駒台（左上）
   - role="board" で盤面を識別、9×9グリッド
   - 先手駒台（右下）
   
3. モーダルオープンボタン
   - 棋譜 / 表示設定 / ショートカット（aria-haspopup="dialog"）
   
4. モーダルダイアログ（非表示、ボタンクリックで表示）
   - 棋譜モーダル（#history-modal、role="dialog" aria-modal="true"）
   - 設定モーダル（#settings-modal）
   - ショートカットモーダル（#shortcuts-modal）
...
.game-container {
  display: grid;
  grid-template-columns: 1fr auto; /* 盤面エリア | 情報パネル */
  gap: 16px;
  max-width: 1400px;
  margin: 0 auto;
}

/* board-area: 駒台と盤面を横並び */
.board-area {
  display: flex;
  align-items: start;
  gap: 8px;
  align-self: start; /* info-panelの高さに引っ張られない */
}
...
│  ご報告の種類 *                              │ legend
│  ○ 不具合報告                               │ radio button
│  ○ 機能リクエスト                           │ radio button
│  ○ 一般的なご意見・ご感想                   │ radio button
│                                             │
│  ℹ️ GitHub Copilot等のAIツールを使うほど、 │ help text
│     より多くのテストケースが生まれ、        │
│     予期しない不具合が発見されることも      │
│     あります。                              │
│                                             │
...
│  メールアドレス（任意）                      │ label
│  ┌─────────────────────────────────────┐  │
│  │ tanaka@example.com                  │  │ email input
│  └─────────────────────────────────────┘  │
│                                             │
│  投稿者の特性（任意）                        │ label
│  ┌─────────────────────────────────────┐  │
...
**アクセシビリティ機能**
- `<h1>`: "ご連絡・ご報告フォーム"
- 全ての form 要素に `aria-label` または `<label>`
- 必須項目: `required` 属性 + `aria-required="true"`
- エラー表示: role="alert" でアナウンス
...
│                                             │
│  メールアドレス                              │ dt
│  tanaka@example.com                         │ dd
│                                             │
│  投稿者の特性                                 │ dt
│  視覚障害                                    │ dd
│                                             │
│  ご意見・ご感想                               │ dt
│  このアプリはすばらしい。もっと多くの人に... │ dd
│                                             │
├─────────────────────────────────────────────┤
│  ┌──────────────┬────────────────────────┐ │
│  │  戻る        │  送信                  │ │ buttons
...
**アクセシビリティ機能**
- aria-live="polite": 「確認画面が表示されました」をアナウンス
- 定義リスト (`<dl>`, `<dt>`, `<dd>`) で内容を構造化
- キーボード操作: Tab で移動可能
...
│                                             │
│                                             │
│     ✓ ご報告ありがとうございます              │ h1 + icon
│                                             │
│  貴重なご意見をお寄せいただき、ありがとう   │ message
│  ございます。                               │
│                                             │
│  いただいたご意見は開発チームで検討させ     │ message
│  ていただきます。                          │
│                                             │
├─────────────────────────────────────────────┤
│  ┌────────────────────────────────────┐   │
│  │  ホーム画面に戻る                   │   │ button (primary)
│  └────────────────────────────────────┘   │
│                                             │
└─────────────────────────────────────────────┘
```
...
**アクセシビリティ機能**
- aria-live="polite": 「ご報告ありがとうございます」をアナウンス
- フォーカス管理: `<h1>` にフォーカス移行
- `role="complementary"`: 完了メッセージを重要情報として標識
...
```
┌─────────────────────────────────────────────┐
│  ヘルプ | ご連絡・ご報告 | プライバシー     │ footer links
└─────────────────────────────────────────────┘
```

**リンク追加**
- "ご連絡・ご報告": /feedback へのリンク
- テキスト: スクリーンリーダーで「ご連絡・ご報告。不具合や機能リクエストを送信できます」
